...
const lambdaOAC = new aws.cloudfront.OriginAccessControl(
  ...
  {
    name: `${$app.name}-${$app.stage}-${createResourceName(
      'OriginAccessControl'
    )}`,
...
const groupOriginId = `${$app.name}-${$app.stage}-${createResourceName(
  'GroupOrigin'
)}`;
const primaryOriginId = `${$app.name}-${$app.stage}-${createResourceName(
  'PrimaryOrigin'
)}`;
const secondaryOriginId = `${$app.name}-${$app.stage}-${createResourceName(
  'SecondaryOrigin'
)}`;
...
const s3Distribution = new sst.aws.Cdn(
  createResourceName('ImageDistribution'),
  {
    ...
    domain: {
      name: 'emage.iosensei.com',
      redirects: ['www.emage.iosensei.com'],
    },
  }
);
